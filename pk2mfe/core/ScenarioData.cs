    ...
    public class ScenarioData
    {
        ...
        public ScenarioData()
        {
            for (int id = 0; id < cityArray.Length; id++)
                cityArray[id] = new City(id);
            for (int id = 0; id < gatePortArray.Length; id++)
                gatePortArray[id] = new GatePort(id + cityCount);
            for (int id = 0; id < provinceArray.Length; id++)
                provinceArray[id] = new Province(id);
            for (int id = 0; id < regionArray.Length; id++)
                regionArray[id] = new Region(id);
        }

        public void LoadFromGlobalScenario(string scenarioPath)
        {
            byte[] data = new byte[GlobalScenario.Size];
            FileStream fs = new FileStream(scenarioPath, FileMode.Open, FileAccess.Read);
            fs.Read(data, 0, data.Length);
            fs.Close();
            GlobalScenario globalScenario = new GlobalScenario();
        ...
        public void SaveToGlobalScenario(string scenarioPath)
        {
            ...
            FileStream fout = new FileStream(scenarioPath, FileMode.Open, FileAccess.Write);
