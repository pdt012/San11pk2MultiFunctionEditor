...
namespace kmfe.Core
{
    public class ScenarioData
    {
        ...
        public const int treasureCount = 900;
        ...
        public const int treasureCustomizeBegin = 0;
        public const int treasureCustomizeEnd = 100;
        ...
        public readonly Treasure[] treasureArray = new Treasure[treasureCount];
        ...
        public ScenarioData()
        {
            ...
            for (int id = 0; id < treasureArray.Length; id++)
                treasureArray[id] = new(id);
        }

        public void LoadFromGlobalScenario(string scenarioPath)
        {
            byte[] data = new byte[S11.S11GlobalScenario.GlobalScenario.Size];
            FileStream fs = new(scenarioPath, FileMode.Open, FileAccess.Read);
            fs.Read(data);
            fs.Close();
            S11.S11GlobalScenario.GlobalScenario globalScenario = new();
            globalScenario.FromBytes(data);

            for (int id = 0; id < cityCount; id++)
            {
                S11.S11GlobalScenario.City s11City = globalScenario.cityArray[id];
            ...
            {
                S11.S11GlobalScenario.GatePort s11Port = globalScenario.gatePortArray[id];
                Town town = townArray[id];
                town.name = CodeConvertHelper.Pk2Str(s11Port.name);
            }
            for (int id = 0; id < provinceCount; id++)
            {
                S11.S11GlobalScenario.Province s11Province = globalScenario.provinceArray[id];
            ...
            for (int id = 0; id < regionArray.Length; id++)
            {
                S11.S11GlobalScenario.Region s11Region = globalScenario.regionArray[id];
            ...
            for (int id = 0; id < titleArray.Length; id++)
            {
                S11.S11GlobalScenario.Title s11Title = globalScenario.titleArray[id];
            ...
            for (int id = 0; id < rankArray.Length; id++)
            {
                S11.S11GlobalScenario.Rank s11Rank = globalScenario.rankArray[id];
        ...
        public void SaveToGlobalScenario(string scenarioPath)
        {
            ...
            {
                S11.S11GlobalScenario.GatePort s11GatePort = globalScenario.gatePortArray[i];
